...
public static class DatabaseExtensions
{
    ...
    /// <summary>
    /// Создает виртуальную таблицу полнотекстового поиска FTS5 и триггеры синхронизации метаданных треков.
    /// </summary>
    /// <param name="context">Активное подключение к базе данных SQLite.</param>
    /// <param name="ct">Токен отмены асинхронной операции.</param>
    /// <returns>Задача, представляющая асинхронную операцию инициализации FTS5.</returns>
    public static async Task EnsureFtsTablesAsync(this SqliteConnection context, CancellationToken ct = default)
    {
        const string sql = """
    ...
    /// <summary>
    /// Выполняет полную очистку и переиндексацию виртуальной таблицы полнотекстового поиска FTS5.
    /// </summary>
    /// <param name="context">Активное подключение к базе данных SQLite.</param>
    /// <param name="ct">Токен отмены асинхронной операции.</param>
    /// <returns>Задача, представляющая асинхронную операцию перестроения индекса FTS5.</returns>
    public static async Task RebuildFtsIndexAsync(this SqliteConnection context, CancellationToken ct = default)
    {
        bool closeOnExit = await EnsureConnectionOpenAsync(context, ct).ConfigureAwait(false);
        try
        {
            await using var cmd = context.CreateCommand();
            cmd.CommandText = "DELETE FROM TracksFts; INSERT INTO TracksFts(rowid, Title, Author) SELECT RowId, Title, Author FROM Tracks;";
            await cmd.ExecuteNonQueryAsync(ct).ConfigureAwait(false);
        }
        finally
        {
